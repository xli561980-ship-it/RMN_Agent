...
## 2026-06-01 — 本地解析默认化 + 递归 ingest + 结果比较闭环

### 做了什么

| 模块 | 调整 |
|------|------|
| `ingest.py` | 递归扫描 `data/papers/` 与 `data/manuals/`；修复 processed 记录存在但 Chroma 无 chunk 的 stale 状态 |
| `ingest.py` | zero-chunk 解析不再写入 processed / manifest，后续 ingest 可重试 |
| `.env.example` / `requirements.txt` / `pyproject.toml` | 默认走本地解析；LlamaParse 保持可选，不作为默认依赖 |
| `eval/run_experiment.py` | 保存每次 golden run 的结构化 JSONL 结果 |
| `eval/compare_runs.py` | 对比 baseline / candidate 的 route、required source、citation、latency 与自动 score |
| `eval/audit_corpus.py` | 对齐磁盘文档、`processed_files.json` 与 Chroma sources |
| `eval/parse_audit.py` | 不写 Chroma 的解析质量抽样审计，可比较本地 fallback 与 LlamaParse |
| `fusion_prompts.py` / `rag_core.py` | 强化 citation 规则；fusion generation 默认 temperature 改为 `0.0` |
| `README.md` / `eval/reports/*` | 记录新的调试流程、完整报告与 run comparison 结果 |

### 为什么

LlamaParse 当前受付费 credit 限制，不适合作为默认解析路线。本轮目标是把调试流程闭环补完整：每次运行可保存、可比较、可审计，并能基于指标选择当前最好的候选，而不是只看单次 Streamlit 回答。

### 效果（本地 corpus，5 题 golden generation run）

| 指标 | 2026-05-31 baseline | 2026-06-01 final |
|------|---------------------|------------------|
| Chroma sources | 275 | 483 |
| Chroma chunks | 7891 | 11715 |
| processed without Chroma | 87 | 0 |
| zero-chunk suspects | 87 | 0 |
| route / required source checks | 5/5 | 5/5 |
| citation validation | 20% | 100% |
| automatic score | 0.7143 | 0.9084 |

关键报告：

- [`eval/reports/full_report_2026-06-01.md`](../eval/reports/full_report_2026-06-01.md)
- [`eval/reports/compare_2026-05-31_vs_2026-06-01_final.md`](../eval/reports/compare_2026-05-31_vs_2026-06-01_final.md)
- [`eval/reports/corpus_audit_2026-06-01.json`](../eval/reports/corpus_audit_2026-06-01.json)

### 设计决策

- 默认不启用 LlamaParse，避免无意消耗付费 credits
- 原始 `eval/runs/*.jsonl` 保持本地-only，不提交到 GitHub，因为其中可能包含检索片段和生成答案
- 保持 citation validator 严格；当前 5 题集已能在 strict prompt + temperature 0 下通过
